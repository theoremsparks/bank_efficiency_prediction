...
# Set Directory
# ==========================================================
OUTPUT_DIR = get_model_output_dir("stacking_optimized_classifier")
MODEL_DIR = get_model_dir("stacking_optimized_classifier")
...
def main():
    print("=" * 70)
    print("RUNNING STACKING OPTIMIZED CLASSIFIER TRAINING")
    print("=" * 70)
    ...
    cv_metrics_df.to_csv(
        OUTPUT_DIR / "stacking_cross_validation_metrics.csv",
        index=False,
    )
    ...
    # ==========================================================
    joblib.dump(stacking_model, MODEL_DIR / "stacking_final_model.joblib")
    ...
    plt.title("Stacking Classifier Confusion Matrix", fontsize=14)
    plt.xlabel("Predicted Class", fontsize=12)
    plt.ylabel("True Class", fontsize=12)
    plt.xticks(rotation=30, ha="right", fontsize=12)
    plt.yticks(rotation=0, fontsize=12)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "stacking_confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.title("Stacking Classifier ROC Curves (One-vs-Rest)", fontsize=16)
    plt.legend(loc="lower right", fontsize=12)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "stacking_multiclass_roc_curves.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    evaluation_metrics_df.to_csv(
        OUTPUT_DIR / "stacking_evaluation_metrics.csv",
        index=False,
    )
    ...
    pred_df.to_csv(OUTPUT_DIR / "stacking_test_predictions.csv", index=False)
    ...
    roc_auc_df.to_csv(OUTPUT_DIR / "stacking_multiclass_roc_auc.csv", index=False)

    print(f"\nSaved ML outputs to: {OUTPUT_DIR}")

    print("=" * 70)
    print("STACKING OPTIMIZED TRAINING COMPLETED")
    print("=" * 70)
