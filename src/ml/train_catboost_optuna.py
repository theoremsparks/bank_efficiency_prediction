...
OUTPUT_DIR = get_model_output_dir("catboost_optuna")
MODEL_DIR = get_model_dir("catboost_optuna")
...
def main():
    print("=" * 70)
    print("RUNNING CATBOOST OPTUNA TRAINING")
    print("=" * 70)
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="catboost_multiclass_optuna_macro_f1",
        sampler=sampler
    )
    ...
    optuna_trials_df.to_csv(
        OUTPUT_DIR / "cat_optuna_trials.csv",
        index=False
    )
    ...
    best_params_df.to_csv(
        OUTPUT_DIR / "cat_best_params.csv",
        index=False
    )

    # ==========================================================
    # Train best CatBoost classifier
    # ==========================================================
    ...
    # ==========================================================
    joblib.dump(cat_model, MODEL_DIR / "cat_final_optimized_model.joblib")
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "cat_confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "cat_multiclass_roc_curves.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    evaluation_metrics_df.to_csv(
        OUTPUT_DIR / "cat_evaluation_metrics.csv",
        index=False
    )
    ...
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "cat_feature_importance.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    mean_abs_shap_df.to_csv(
        OUTPUT_DIR / "cat_shap_stacked_mean_importance.csv",
        index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "cat_shap_stacked_mean_importance.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
    ...
    pred_df.to_csv(OUTPUT_DIR / "cat_test_predictions.csv", index=False)

    # ==========================================================
    # Save feature importance
    # ==========================================================
    feature_importance.to_csv(OUTPUT_DIR / "cat_feature_importance.csv", index=False)
    ...
    roc_auc_df.to_csv(OUTPUT_DIR / "cat_multiclass_roc_auc.csv", index=False)
