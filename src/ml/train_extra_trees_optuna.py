...
OUTPUT_DIR = get_model_output_dir("extra_trees_optuna")
MODEL_DIR = get_model_dir("extra_trees_optuna")
...
def main():
    print("=" * 70)
    print("RUNNING EXTRA TREES OPTUNA TRAINING")
    print("=" * 70)
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="et_multiclass_optuna_macro_f1",
        sampler=sampler
    )
    ...
    optuna_trials_df.to_csv(
        OUTPUT_DIR / "et_optuna_trials.csv",
        index=False
    )
    ...
    best_params_df.to_csv(
        OUTPUT_DIR / "et_best_params.csv",
        index=False
    )

    # ==========================================================
    # Train best Extra Trees classifier
    # ==========================================================
    et_model = ExtraTreesClassifier(
        **study.best_params,
        random_state=42,
    ...
    # ==========================================================
    joblib.dump(et_model, MODEL_DIR / "et_final_optimized_model.joblib")
    ...
    plt.title("Extra Trees Confusion Matrix", fontsize=14)
    plt.xlabel("Predicted Class", fontsize=12)
    plt.ylabel("True Class", fontsize=12)
    plt.xticks(rotation=30, ha="right", fontsize=12)
    plt.yticks(rotation=0, fontsize=12)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "et_confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "et_multiclass_roc_curves.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    evaluation_metrics_df.to_csv(
        OUTPUT_DIR / "et_evaluation_metrics.csv",
        index=False
    )
    ...
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "et_feature_importance.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    mean_abs_shap_df.to_csv(
        OUTPUT_DIR / "et_shap_stacked_mean_importance.csv",
        index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "et_shap_stacked_mean_importance.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
    ...
    pred_df.to_csv(OUTPUT_DIR / "et_test_predictions.csv", index=False)

    # ==========================================================
    # Save feature importance
    # ==========================================================
    feature_importance.to_csv(OUTPUT_DIR / "et_feature_importance.csv", index=False)
    ...
    roc_auc_df.to_csv(OUTPUT_DIR / "et_multiclass_roc_auc.csv", index=False)
