...
OUTPUT_DIR = get_model_output_dir("xgboost_optuna")
MODEL_DIR = get_model_dir("xgboost_optuna")
...
def main():
    print("=" * 70)
    print("RUNNING XGBOOST OPTUNA TRAINING")
    print("=" * 70)
    ...
    y = y_original - 1  # XGBoost needs 0-based labels
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="xgb_multiclass_optuna_macro_f1",
        sampler=sampler
    )
    ...
    optuna_trials_df.to_csv(
        OUTPUT_DIR / "xgb_optuna_trials.csv",
        index=False
    )
    ...
    best_params_df.to_csv(
        OUTPUT_DIR / "xgb_best_params.csv",
        index=False
    )
    ...
    # ==========================================================
    joblib.dump(xgb_model, MODEL_DIR / "xgb_final_optimized_model.joblib")
    ...
    # Convert back to original class labels 1,2,3,4
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "xgb_multiclass_roc_curves.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    evaluation_metrics_df.to_csv(
        OUTPUT_DIR / "xgb_evaluation_metrics.csv",
        index=False
    )
    ...
    mean_abs_shap_df.to_csv(
        OUTPUT_DIR / "xgb_shap_stacked_mean_importance.csv",
        index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "xgb_shap_stacked_mean_importance.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
    ...
    roc_auc_df.to_csv(OUTPUT_DIR / "xgb_multiclass_roc_auc.csv", index=False)

    print(f"\nSaved ML outputs to: {OUTPUT_DIR}")
    print("=" * 70)
    print("XGOOST TRAINING COMPLETED")
    print("=" * 70)
