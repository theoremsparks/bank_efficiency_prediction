...
OUTPUT_DIR = get_model_output_dir("adaboost_optuna")
MODEL_DIR = get_model_dir("adaboost_optuna")
...
def main():
    print("=" * 70)
    print("RUNNING ADABOOST TRAINING")
    print("=" * 70)
    ...
    def objective(trial):
        base_estimator = DecisionTreeClassifier(
            max_depth=trial.suggest_int("base_max_depth", 5, 50),
            min_samples_split=trial.suggest_int("base_min_samples_split", 2, 20),
            min_samples_leaf=trial.suggest_int("base_min_samples_leaf", 2, 8),
            random_state=42,
        )
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="ada_multiclass_optuna_macro_f1",
        sampler=sampler,
    )
    ...
    optuna_trials_df.to_csv(
        OUTPUT_DIR / "ada_optuna_trials.csv",
        index=False,
    )
    ...
    best_params_df.to_csv(
        OUTPUT_DIR / "ada_best_params.csv",
        index=False,
    )

    # ==========================================================
    # Train best AdaBoost classifier
    # ==========================================================
    best_base_estimator = DecisionTreeClassifier(
        max_depth=study.best_params["base_max_depth"],
        min_samples_split=study.best_params["base_min_samples_split"],
        min_samples_leaf=study.best_params["base_min_samples_leaf"],
        random_state=42,
    )
    ...
    # ==========================================================
    joblib.dump(ada_model, MODEL_DIR / "ada_final_optimized_model.joblib")
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "ada_confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "ada_multiclass_roc_curves.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    evaluation_metrics_df.to_csv(
        OUTPUT_DIR / "ada_evaluation_metrics.csv",
        index=False,
    )
    ...
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "ada_feature_importance.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    pred_df.to_csv(OUTPUT_DIR / "ada_test_predictions.csv", index=False)

    # ==========================================================
    # Save feature importance
    # ==========================================================
    feature_importance.to_csv(OUTPUT_DIR / "ada_feature_importance.csv", index=False)
    ...
    roc_auc_df.to_csv(OUTPUT_DIR / "ada_multiclass_roc_auc.csv", index=False)
